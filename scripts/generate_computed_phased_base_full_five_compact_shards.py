#!/usr/bin/env python3
"""Generate recursive five-block Taylor shards on ``[2, 5/2]``."""
...
from generate_computed_phased_base_lower_four_compact_shards import (
...
def stem(cell: int, index: int) -> str:
    return f"computedPhasedBaseFullFiveCompactCell{cell}Shard{index}"
...
def render_shard(cell: int, index: int, lower: Q, upper: Q,
    ...
    previous = previous_override or (
        f"computedPhasedBaseLowerFourCompactCell{cell}Shard{index}"
    )
    previous_import = import_override or (
        f"BoundaryComputedPhasedBaseLowerFourCompactCell{cell}Shard{index}"
    )
    ...
    return f"""import RiemannVenue.Venue.BoundaryComputedPhasedBaseFullFiveBlockCore
...
def {name}Trig (b : Fin 5) (g : Fin 20) : RationalTrigInterval :=
  ![{previous}Trig 0 g, {previous}Trig 1 g, {previous}Trig 2 g,
    {previous}Trig 3 g,
    RationalTrigInterval.add ({previous}Trig 3 g)
...
theorem {name}Trig_contains (b : Fin 5) (g : Fin 20) :
  ...
  · change ({previous}Trig 2 g).Contains _
    ...
    norm_num
  · change ({previous}Trig 3 g).Contains _
    ...
    simp [computedPhasedBaseFullFiveColumn_translation,
      ...
      {name}Interval, {previous}Interval]
    norm_num
  · change (RationalTrigInterval.add ({previous}Trig 3 g)
      (computedPhasedBaseMiddleHalfShiftTrig g)).Contains _
    have h := RationalTrigInterval.contains_add
      ({previous}Trig_contains 3 g)
...
theorem {name}Bump_contains (b : Fin 5) (n : Fin 15) (x : ℝ)
  ...
  · change ({previous}Bump 2 n).Contains
    ...
    rw [RationalInterval.Contains] at hx ⊢
    convert hx using 1 <;> norm_num [{name}Interval, {previous}Interval] <;> ring
  · change ({previous}Bump 3 n).Contains
...
theorem {name}PointBump_contains (b : Fin 5) (n : Fin 15) :
  ...
  · change ({previous}PointBump 2 n).Contains
    ...
    convert {previous}PointBump_contains 2 n using 1 <;>
      norm_num [{name}Interval, {previous}Interval]
  · change ({previous}PointBump 3 n).Contains
...
def render_packet(cell: int) -> str:
    ...
    imports = [
        f"import RiemannVenue.Venue.BoundaryComputedPhasedBaseFullFiveCompactCell{cell}Shard{i}"
        for _, i, *_ in group
    ]
    packet = f"computedPhasedBaseFullFiveCompactCell{cell}TaylorPacket"
    return "\n".join([
...
def main() -> None:
    ...
    for cell, index, lower, upper in entries():
        ...
        write_if_changed(
            args.output_dir /
            f"BoundaryComputedPhasedBaseFullFiveCompactCell{cell}Shard{index}.lean",
    ...
    if args.index is None:
        for cell in range(7):
            if args.cell is None or args.cell == cell:
                write_if_changed(
                    args.output_dir /
                    f"BoundaryComputedPhasedBaseFullFiveCompactTaylorCell{cell}.lean",
                    render_packet(cell),
                )
    if args.cell is None and args.index is None:
        write_if_changed(
            args.output_dir / "BoundaryComputedPhasedBaseFullFiveTailTaylor.lean",
            render_shard(
                0, 0, Q(1115, 448), Q(5, 2),
                name_override="computedPhasedBaseFullFiveTail",
                previous_override="computedPhasedBaseLowerFourTail",
                import_override="BoundaryComputedPhasedBaseLowerFourTailTaylor",
            ).replace(
                "/-! Generated recursive five-block shard on `[2, 5/2]`. -/",
        ...
        write_if_changed(
            args.output_dir / "BoundaryComputedPhasedBaseFullFiveCompactShards.lean",
            "\n".join(
                "import RiemannVenue.Venue."
                f"BoundaryComputedPhasedBaseFullFiveCompactCell{c}Shard{i}"
                for c, i, *_ in entries()
            ) + "\n\n/-! Generated import surface for all five-block shards. -/\n",
        )
        write_if_changed(
            args.output_dir /
            "BoundaryComputedPhasedBaseFullFiveCompactTaylorPackets.lean",
            "\n".join(
                "import RiemannVenue.Venue."
                f"BoundaryComputedPhasedBaseFullFiveCompactTaylorCell{c}"
                for c in range(7)
