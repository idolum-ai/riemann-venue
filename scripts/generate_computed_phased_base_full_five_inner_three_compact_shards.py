#!/usr/bin/env python3
"""Generate recursive five-block Taylor shards on ``[1/2, 1]``."""
...
from generate_computed_phased_base_full_five_inner_two_compact_shards import (
...
def stem(cell: int, index: int) -> str:
    return f"computedPhasedBaseFullFiveInnerThreeCompactCell{cell}Shard{index}"
...
def render_shard(cell: int, index: int, lower: Q, upper: Q,
    ...
    previous = previous_override or (
        f"computedPhasedBaseFullFiveInnerTwoCompactCell{cell}Shard{index}"
    )
    previous_import = import_override or (
        f"BoundaryComputedPhasedBaseFullFiveInnerTwoCompactCell{cell}Shard{index}"
    )
...
def render_packet(cell: int) -> str:
    ...
    imports = [
        f"import RiemannVenue.Venue.BoundaryComputedPhasedBaseFullFiveInnerThreeCompactCell{cell}Shard{i}"
        for _, i, *_ in group
    ]
    packet = f"computedPhasedBaseFullFiveInnerThreeCompactCell{cell}TaylorPacket"
    return "\n".join([
...
def main() -> None:
    ...
    for cell, index, lower, upper in entries():
        ...
        write_if_changed(
            args.output_dir /
            f"BoundaryComputedPhasedBaseFullFiveInnerThreeCompactCell{cell}Shard{index}.lean",
    ...
    if args.index is None:
        for cell in range(7):
            if args.cell is None or args.cell == cell:
                write_if_changed(
                    args.output_dir /
                    f"BoundaryComputedPhasedBaseFullFiveInnerThreeCompactTaylorCell{cell}.lean",
                    render_packet(cell),
                )
    if args.cell is None and args.index is None:
        write_if_changed(
            args.output_dir / "BoundaryComputedPhasedBaseFullFiveInnerThreeTailTaylor.lean",
            render_shard(
                0, 0, Q(443, 448), Q(1),
                name_override="computedPhasedBaseFullFiveInnerThreeTail",
                previous_override="computedPhasedBaseFullFiveInnerTwoTail",
                import_override="BoundaryComputedPhasedBaseFullFiveInnerTwoTailTaylor",
            ).replace(
                "/-! Generated recursive five-block shard on `[1/2, 1]`. -/",
        ...
        write_if_changed(
            args.output_dir / "BoundaryComputedPhasedBaseFullFiveInnerThreeCompactShards.lean",
            "\n".join(
                "import RiemannVenue.Venue."
                f"BoundaryComputedPhasedBaseFullFiveInnerThreeCompactCell{c}Shard{i}"
                for c, i, *_ in entries()
            ) + "\n\n/-! Generated import surface for all full five-block inner shards. -/\n",
        )
        write_if_changed(
            args.output_dir /
            "BoundaryComputedPhasedBaseFullFiveInnerThreeCompactTaylorPackets.lean",
            "\n".join(
                "import RiemannVenue.Venue."
                f"BoundaryComputedPhasedBaseFullFiveInnerThreeCompactTaylorCell{c}"
                for c in range(7)
