#!/usr/bin/env python3
"""Generate recursive five-block Taylor shards on ``[3/2, 2]``."""
...
from generate_computed_phased_base_full_five_compact_shards import (
...
def stem(cell: int, index: int) -> str:
    return f"computedPhasedBaseFullFiveInnerOneCompactCell{cell}Shard{index}"
...
def render_shard(cell: int, index: int, lower: Q, upper: Q,
    ...
    previous = previous_override or (
        f"computedPhasedBaseFullFiveCompactCell{cell}Shard{index}"
    )
    previous_import = import_override or (
        f"BoundaryComputedPhasedBaseFullFiveCompactCell{cell}Shard{index}"
    )
...
def render_packet(cell: int) -> str:
    ...
    imports = [
        f"import RiemannVenue.Venue.BoundaryComputedPhasedBaseFullFiveInnerOneCompactCell{cell}Shard{i}"
        for _, i, *_ in group
    ]
    packet = f"computedPhasedBaseFullFiveInnerOneCompactCell{cell}TaylorPacket"
    return "\n".join([
...
def main() -> None:
    ...
    for cell, index, lower, upper in entries():
        ...
        write_if_changed(
            args.output_dir /
            f"BoundaryComputedPhasedBaseFullFiveInnerOneCompactCell{cell}Shard{index}.lean",
    ...
    if args.index is None:
        for cell in range(7):
            if args.cell is None or args.cell == cell:
                write_if_changed(
                    args.output_dir /
                    f"BoundaryComputedPhasedBaseFullFiveInnerOneCompactTaylorCell{cell}.lean",
                    render_packet(cell),
                )
    if args.cell is None and args.index is None:
        write_if_changed(
            args.output_dir / "BoundaryComputedPhasedBaseFullFiveInnerOneTailTaylor.lean",
            render_shard(
                0, 0, Q(891, 448), Q(2),
                name_override="computedPhasedBaseFullFiveInnerOneTail",
                previous_override="computedPhasedBaseFullFiveTail",
                import_override="BoundaryComputedPhasedBaseFullFiveTailTaylor",
            ).replace(
                "/-! Generated recursive five-block shard on `[3/2, 2]`. -/",
        ...
        write_if_changed(
            args.output_dir / "BoundaryComputedPhasedBaseFullFiveInnerOneCompactShards.lean",
            "\n".join(
                "import RiemannVenue.Venue."
                f"BoundaryComputedPhasedBaseFullFiveInnerOneCompactCell{c}Shard{i}"
                for c, i, *_ in entries()
            ) + "\n\n/-! Generated import surface for all full five-block inner shards. -/\n",
        )
        write_if_changed(
            args.output_dir /
            "BoundaryComputedPhasedBaseFullFiveInnerOneCompactTaylorPackets.lean",
            "\n".join(
                "import RiemannVenue.Venue."
                f"BoundaryComputedPhasedBaseFullFiveInnerOneCompactTaylorCell{c}"
                for c in range(7)
