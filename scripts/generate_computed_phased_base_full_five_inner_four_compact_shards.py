#!/usr/bin/env python3
"""Generate recursive five-block Taylor shards on ``[0, 1/2]``."""
...
from generate_computed_phased_base_full_five_inner_three_compact_shards import (
...
def stem(cell: int, index: int) -> str:
    return f"computedPhasedBaseFullFiveInnerFourCompactCell{cell}Shard{index}"
...
def render_shard(cell: int, index: int, lower: Q, upper: Q,
    ...
    previous = previous_override or (
        f"computedPhasedBaseFullFiveInnerThreeCompactCell{cell}Shard{index}"
    )
    previous_import = import_override or (
        f"BoundaryComputedPhasedBaseFullFiveInnerThreeCompactCell{cell}Shard{index}"
    )
...
def render_packet(cell: int) -> str:
    ...
    imports = [
        f"import RiemannVenue.Venue.BoundaryComputedPhasedBaseFullFiveInnerFourCompactCell{cell}Shard{i}"
        for _, i, *_ in group
    ]
    packet = f"computedPhasedBaseFullFiveInnerFourCompactCell{cell}TaylorPacket"
    return "\n".join([
...
def main() -> None:
    ...
    for cell, index, lower, upper in entries():
        ...
        write_if_changed(
            args.output_dir /
            f"BoundaryComputedPhasedBaseFullFiveInnerFourCompactCell{cell}Shard{index}.lean",
    ...
    if args.index is None:
        for cell in range(7):
            if args.cell is None or args.cell == cell:
                write_if_changed(
                    args.output_dir /
                    f"BoundaryComputedPhasedBaseFullFiveInnerFourCompactTaylorCell{cell}.lean",
                    render_packet(cell),
                )
    if args.cell is None and args.index is None:
        write_if_changed(
            args.output_dir / "BoundaryComputedPhasedBaseFullFiveInnerFourTailTaylor.lean",
            render_shard(
                0, 0, Q(219, 448), Q(1, 2),
                name_override="computedPhasedBaseFullFiveInnerFourTail",
                previous_override="computedPhasedBaseFullFiveInnerThreeTail",
                import_override="BoundaryComputedPhasedBaseFullFiveInnerThreeTailTaylor",
            ).replace(
                "/-! Generated recursive five-block shard on `[0, 1/2]`. -/",
        ...
        write_if_changed(
            args.output_dir / "BoundaryComputedPhasedBaseFullFiveInnerFourCompactShards.lean",
            "\n".join(
                "import RiemannVenue.Venue."
                f"BoundaryComputedPhasedBaseFullFiveInnerFourCompactCell{c}Shard{i}"
                for c, i, *_ in entries()
            ) + "\n\n/-! Generated import surface for all full five-block inner shards. -/\n",
        )
        write_if_changed(
            args.output_dir /
            "BoundaryComputedPhasedBaseFullFiveInnerFourCompactTaylorPackets.lean",
            "\n".join(
                "import RiemannVenue.Venue."
                f"BoundaryComputedPhasedBaseFullFiveInnerFourCompactTaylorCell{c}"
                for c in range(7)
