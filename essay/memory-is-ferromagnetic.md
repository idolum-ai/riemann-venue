...
In 1976, Daniel Mattis built a fake spin glass. Take couplings
...
This line is not decoration; it is where computation gets expensive.
...
antiferromagnet. Even *detecting* the disguise is hard in general:
computing how far a sign pattern is from gauge-trivial is itself NP-hard.
The frustrated part of a matrix is not a nuisance term. It is where the
problem keeps its difficulty.
...
showed that the update rule of a modern continuous Hopfield network is the
attention mechanism of a transformer. Attention is not *like* an
associative memory; under the correspondence it *is* one. So a
transformer's matrices inherit the physics vocabulary, and the frustration
question becomes well-posed: multiply the signs around loops of a trained
weight matrix and see what survives conjugation.
...
The proposal is a measurement, and it is cheap. For any weight or
attention matrix: find the diagonal sign gauge that maximizes the positive
part, and report the frustration that remains — the mass on loops that no
gauge clears. Then look at where it sits. Do copy heads and induction
...
The essay's claim is falsifiable in an afternoon, which is the only kind
of claim a hypothesis essay should make.
...
technology collapsed into the physics of one positive valley. The lesson
was not that cancellation is never real. It was that you should check,
because a matrix that acts hostile is sometimes only frustrated — and
sometimes not even that.

Matrix multiplication is, at this point, the substrate of most of what
civilization computes, and the same three-line test applies to all of it.
What part of a trained matrix is a coordinate choice, and what part is
meaning? The gauge knows. Memory is ferromagnetic. Whatever thought is,
it lives in the part that refuses to reconcile.
