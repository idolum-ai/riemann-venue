...
| **The essay's phase boundary**: for `0 < σ ≤ 1/2` the product Poisson measure over the primes is mutually singular with product Haar (ℝ-model of the circle); the `σ > 1/2` equivalence half awaits M5 | proved in Lean | `RiemannVenue/Kakutani/PoissonSingular.lean` (`poissonProduct_mutuallySingular`) |
