...
* `Measure.hellingerAffinity_eq_lintegral_rnDeriv` — under `μ ≪ ν` the
  affinity is `∫⁻ (dμ/dν)^(1/2) dν`.
...
/-- Under `μ ≪ ν` the affinity is the `ν`-integral of `√(dμ/dν)`. -/
theorem hellingerAffinity_eq_lintegral_rnDeriv
  ...
  rw [hellingerAffinity_eq_lintegral_rnDeriv_mul_rnDeriv ν hμν .rfl]
  refine lintegral_congr_ae ?_
